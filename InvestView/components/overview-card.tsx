...
import { getCompositePercentile, getValuationZone, valuationZoneMap } from "@/components/valuation-utils";
...
export function OverviewCard({ asset }: OverviewCardProps) {
  const zone = getValuationZone(asset);
  const zoneStyle = valuationZoneMap[zone];
  const isUp = asset.dailyChangePct >= 0;
  const compositePercentile = getCompositePercentile(asset);
  ...
  return (
    <article className={`rounded-xl border p-4 shadow-terminal transition ${zoneStyle.card}`}>
      ...
      <dl className="space-y-3 text-sm">
        ...
        <div className="flex justify-between">
          <dt className="text-slate-400">3/5/10年分位</dt>
          <dd className="font-medium text-slate-100">
            {asset.pePercentile3y}% / {asset.pePercentile5y}% / {asset.pePercentile10y}%
          </dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-slate-400">综合分位(3/5/10)</dt>
          <dd className="font-semibold text-cyan-200">{compositePercentile}%</dd>
        </div>
      </dl>
    </article>
  );
